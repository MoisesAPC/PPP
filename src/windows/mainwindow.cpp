...
void MainWindow::fileOpenMenu() {
    ...
    QString filename = QFileDialog::getOpenFileName(this, "Open File", getLastOpenedDirectory(settings), "All accepted filetypes (*.pak *.mpak *.note *.eep);;Individual note (*.note);;Controller Pak data (*.pak *.mpak);;Cartridge (Japanese version only) (*.eep);;All Files (*)");
...
void MainWindow::fileSaveAsMenu() {
    QString filepath = QFileDialog::getSaveFileName(this, "Save As...", QString(), "All accepted filetypes (*.pak, *.mpak, *.note, *.eep);;Individual note (*.note);;Controller Pak data (*.pak, *.mpak);;Cartridge (Japanese version only) (*.eep);;All Files (*)");
